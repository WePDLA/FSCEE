...
from models.modules.similarity_scorer_base import SimilarityScorerBase, MatchingSimilarityScorer, \
    PrototypeSimilarityScorer,MinSimilarityScorer, FeatureMinSimilarityScorer,PrototypeFeatureSimilarityScorer, ProtowithAttGCNSimilarityScorer,ProtoWithLabelSimilarityScorer, TapNetSimilarityScorer, \
    reps_dot, reps_l2_sim, reps_cosine_sim,ProtowithCrossSimilarityScorer,TapNetCrossSimilarityScorer
from models.modules.emission_scorer_base import EmissionScorerBase, MNetEmissionScorer, TapNetCrossEmissionScorer, \
    PrototypeEmissionScorer, PrototypeFeatureEmissionScorer, ProtowithAttGCNEmissionScorer,ProtoWithLabelEmissionScorer, TapNetEmissionScorer,ProtowithCrossEmissionScorer,MinEmissionScorer,FeatureMinEmissionScorer
...
def make_model(opt, config):
    ...
    if opt.emission == 'mnet':
    ...
    elif opt.emission == 'proto_feature':
        similarity_scorer = PrototypeFeatureSimilarityScorer(opt=opt,num_tag = config['num_tags'] ,sim_func=sim_func, emb_log=emb_log)
        emission_scorer = PrototypeFeatureEmissionScorer(similarity_scorer, ems_scaler)
    
    elif opt.emission == 'Min':
        similarity_scorer = MinSimilarityScorer(opt=opt,sim_func=sim_func, emb_log=emb_log)
        emission_scorer = MinEmissionScorer(similarity_scorer, ems_scaler)
    elif opt.emission == 'feature_Min':
        similarity_scorer = FeatureMinSimilarityScorer(opt=opt,sim_func=sim_func, emb_log=emb_log)
        emission_scorer = FeatureMinEmissionScorer(similarity_scorer, ems_scaler)

    elif opt.emission == 'proto_cross_attention':
        similarity_scorer = ProtowithCrossSimilarityScorer(sim_func=sim_func, emb_log=emb_log)
        emission_scorer = ProtowithCrossEmissionScorer(similarity_scorer, ems_scaler)
    ...
    elif opt.emission == 'proto_with_AttGRU':
        similarity_scorer = ProtowithAttGCNSimilarityScorer(opt=opt,sim_func=sim_func,  emb_log=emb_log)
        emission_scorer = ProtowithAttGCNEmissionScorer(similarity_scorer, ems_scaler)
    ...
    elif opt.emission == "tapnet_cross":
        # set num of anchors:
        # (1) if provided in config, use it (usually in load model case.)
        # (2) *3 is used to ensure enough anchors ( > num_tags of unseen domains )
        num_anchors = config['num_anchors'] if 'num_anchors' in config else config['num_tags'] * 3
        config['num_anchors'] = num_anchors
        anchor_dim = 256 if opt.context_emb == 'electra' else 768
        similarity_scorer = TapNetCrossSimilarityScorer(
            sim_func=sim_func, num_anchors=num_anchors, mlp_out_dim=opt.tap_mlp_out_dim,
            random_init=opt.tap_random_init, random_init_r=opt.tap_random_init_r,
            mlp=opt.tap_mlp, emb_log=emb_log, tap_proto=opt.tap_proto, tap_proto_r=opt.tap_proto_r,
            anchor_dim=anchor_dim)
        emission_scorer = TapNetCrossEmissionScorer(similarity_scorer, ems_scaler)
    else:
        raise TypeError('wrong component type')
