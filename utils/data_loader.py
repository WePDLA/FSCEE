...
DataItem = collections.namedtuple("DataItem", ["seq_in", "seq_out", "label","adj"])
...
class FewShotRawDataLoader(RawDataLoaderBase):
    ...
    def raw_data2examples(self, raw_data: Dict) -> (List[FewShotExample], List[List[FewShotExample]], int):
        ...
        all_support_size = []
        few_shot_batches = []
        for task_n, task in raw_data.items():
            # 对于每一个领域  有50/100 episodes 的support query
            # Notice: the batch here means few shot batch, not training batch
            for batch_id, batch in enumerate(task):
                one_batch_examples = []
    ...
    def get_data_items(self, parts: dict) -> List[DataItem]:
        data_item_lst = []
        # print(parts,DataItem)
        for seq_in, seq_out, label,adj in zip(parts['seq_ins'], parts['seq_outs'], parts['labels'],parts["adj"]):
            ...
            data_item = DataItem(seq_in=seq_in, seq_out=seq_out, label=label,adj=adj)
