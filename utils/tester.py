...
class TesterBase:
    ...
    def do_test(self, model: torch.nn.Module, test_features: List[FewShotFeature], id2label: dict,
        ...
        F1,P,R = self.eval_predictions(all_results, id2label, log_mark)
        return F1,P,R 
...
class FewShotTester(TesterBase):
    ...
    def eval_predictions(self, all_results: List[RawResult], id2label: dict, log_mark: str) -> float:
        ...
        for b_id, fs_batch in all_batches:
            precision, recall, f1 = self.eval_one_few_shot_batch(b_id, fs_batch, id2label, log_mark)
            all_scores_F1.append(f1)
            all_scores_P.append(precision)
            all_scores_R.append(recall)
            gc.collect()
        return sum(all_scores_F1) * 1.0 / len(all_scores_F1),sum(all_scores_P) * 1.0 / len(all_scores_P),sum(all_scores_R) * 1.0 / len(all_scores_R)
    ...
    def eval_with_script(self, output_prediction_file):
        ...
        with open(output_prediction_file,"r") as fread:
            precision, recall, f1 = evaluate_conll_file(fread)
        # result_path = output_prediction_file.replace(".txt",".res.txt")
        # os.system("perl {} < {} > {}".format(self.opt.eval_script,output_prediction_file,result_path))
        # time.sleep(60)
        #     # logger.info("result_path:{}".format(result_path))
        # with open(result_path, 'r') as f:
        #     # p = subprocess.Popen(script_args, stdout=subprocess.PIPE, stdin=res_file,close_fds=True)
        #     # p.wait()
...
def eval_check_points(opt, tester, test_features, test_id2label, device):
    ...
    for cpt_file in all_cpt_file:
        ...
        F1,P,R  = tester.do_test(testing_model, test_features, test_id2label, log_mark='test_pred')
        if F1 > max_score:
            max_score = F1
        logger.info('cpt_file:{} - test F1:{} , P: {} , R :{}'.format(cpt_file, F1,P,R))
    return max_score
