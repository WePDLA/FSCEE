...
class TrainerBase:
    ...
    def model_selection(self, model, best_score, dev_features, dev_id2label, test_features=None, test_id2label=None):
        ...
        F1,P,R = self.tester.do_test(dev_model, dev_features, dev_id2label, log_mark='dev_pred')
        logger.info("  dev score(F1) = {} , P = {}, R = {} ".format(F1,P,R))
        print("  dev score(F1) = {} , P = {}, R = {} ".format(F1,P,R))
        best_model = None
        test_F1 = None
        if F1 > best_score:
            ...
            if test_features:
                ...
                test_F1,P,R = self.tester.do_test(test_model, test_features, test_id2label, log_mark='test_pred')
                logger.info("  test score(F1) = {} , P = {},R = {}".format(test_F1,P,R))
                print(" test score(F1) = {} , P = {},R = {}".format(test_F1,P,R))
        # reset the model status
        model.train()
        return F1, test_F1, best_model
...
def prepare_optimizer(opt, model, num_train_features, upper_structures=None):
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=opt.learning_rate, correct_bias=True)
