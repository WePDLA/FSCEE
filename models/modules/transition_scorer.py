    ...
    def init_backoff_trans(self):
        if self.backoff_init == 'rand':
            ...
            self.backoff_trans_mat = nn.Parameter(torch.tensor(
                [[0.8, 0.2, -0.5, 0.01, -0.5],
                 [0.5, 0.01, 0.5, 0.01, -0.5],
                 [0.8, 0.01, 0.1, 0.01, -0.5]]), requires_grad=True)
            self.backoff_start_trans_mat = nn.Parameter(torch.tensor([0.8, 0.2, -0.5]), requires_grad=True)
            self.backoff_end_trans_mat = nn.Parameter(torch.tensor([0.8, 0.2, 0.5]), requires_grad=True)
        else:
