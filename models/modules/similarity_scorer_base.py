...
class PrototypeFeatureSimilarityScorer(SimilarityScorerBase):
    ...
    def forward(
        ...
        test_reps = att_test_reps  # shape (batch_size, sent_len, emb_dim)
        # print("============",support_size,test_reps.shape,att_support_reps.shape)
        # weight = torch.bmm(test_reps,torch.transpose(att_support_reps, -1, -2)) # shape (batch_size, test_len, support_len)
        # support_targets = support_targets.view(batch_size, support_len * support_size, num_tags).float()
        # sim_score = torch.bmm(weight,support_targets)  # shape (batch_size, test_len, num_tags)
...
class MinSimilarityScorer(SimilarityScorerBase):
    ...
    def forward(
        ...
        support_targets = support_targets.view(batch_size, support_len * support_size, num_tags).float()
        sim_score = torch.bmm(weight,support_targets)  # shape (batch_size, test_len, num_tags)
        # print(sim_score.shape)    # shape (batch_size, test_len, support_len)
        # print(xxxx)
    ...
    def remove_0(self, my_tensor):
        return my_tensor + 0.0001
class FeatureMinSimilarityScorer(SimilarityScorerBase):
    def __init__(self, opt,sim_func, emb_log=None):
        super(FeatureMinSimilarityScorer, self).__init__(sim_func=sim_func, emb_log=emb_log)
        # self.fc = nn.Linear(opt.emb_dim, opt.emb_dim, bias=True)
        if opt.gcns == 1:
            self.multihead_attn = nn.MultiheadAttention(opt.emb_dim+opt.gcn_emb, 1)
        else:
