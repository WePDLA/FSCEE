...
class ConditionalRandomField(torch.nn.Module):
    ...
    def _input_likelihood(self,
                          ...
                          mask: torch.Tensor,log_numerator) -> torch.Tensor:
        ...
        mask = mask.float().transpose(0, 1).contiguous()  # (len,batch_size)
        logits = logits.transpose(0, 1).contiguous()   # size (sequence_length,batch_size, num_tags)
        ...
        if self.include_start_end_transitions:
            ...
            alpha = logits[0]   # 第0个元素 （batch_size, num_tags)
        ...
        for i in range(1, sequence_length):
            ...
            # temp = broadcast_alpha + emit_scores
            # print(inner,log_numerator)
            # if inner == log_numerator:
            #     print(log_numerator)
            # print("----------------------------------------------",len(mask))
            ...
            # print("====================================",len(mask),i)
            if i < len(mask):                #  util.logsumexp(inner, 1) shape(batch_size , num_tags)
                alpha = (util.logsumexp(inner, 1) * mask[i].view(batch_size, 1) +
                        alpha * (1 - mask[i]).view(batch_size, 1))
            else:
                alpha = (util.logsumexp(inner, 1) * mask[i-1].view(batch_size, 1) +
                         alpha * (1 - mask[i-1]).view(batch_size, 1))
            # print(log_numerator,util.logsumexp(inner, 1) )
    ...
    def _joint_likelihood(self,
        ...
        if self.include_start_end_transitions:
            ...
            score = 0.0
        # print("sequence_length----------------------------------s",len(mask),sequence_length)
