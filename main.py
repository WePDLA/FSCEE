...
def main():
    ...
    preprocessor = make_preprocessor(opt)  # context_emb:   'bert', 'sep_bert', 'electra' 'elmo'  'glove', 'raw'
    if opt.do_train:
        train_features, train_label2id, train_id2label, dev_features, dev_label2id, dev_id2label = \
            get_training_data_and_feature(opt, data_loader, preprocessor)

        if opt.mask_transition and opt.task == 'sl':  # sl  means Sequence labeling 序列标注任务
            opt.train_label_mask = make_label_mask(opt, opt.train_path, train_label2id)  #disable cross domain transition
    ...
    trainer_class = SchemaFewShotTrainer if opt.use_schema else FewShotTrainer   # use_schema：Divide emission by each tag's token num in support set
