...
class SLUDataLoader(RawDataLoaderBase):
    ...
    def load_data(self, path: str) -> dict:
        ...
        if self.opt.intent_as_domain and self.opt.task == 'sl':  # re-split data by regarding label/intent as domain
            domain_data = {}
            for index,domain in enumerate( all_data["labels"]):
                if domain[0] not in domain_data:
                    domain_data[domain[0]] = { 'seq_ins':[all_data["seq_ins"][index]], 'labels':[all_data["labels"][index]], 'seq_outs':[all_data["seq_outs"][index]] }
                else:
                    domain_data[domain[0]]['seq_ins'].append(all_data["seq_ins"][index])
                    domain_data[domain[0]]['labels'].append(all_data["labels"][index])
                    domain_data[domain[0]]['seq_outs'].append(all_data["seq_outs"][index])

            return self.split_data_by_intent(domain_data)
